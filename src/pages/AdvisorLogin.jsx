...
import { advisorLogin } from "../api"; // Replace with actual API call
...
const AdvisorLogin = () => {
    ...
    const handleLogin = async (e) => {
        e.preventDefault();
        try {
            const res = await advisorLogin(email, password);
            if (res.success) {
                localStorage.setItem("token", res.token);
                navigate("/advisor-dashboard"); // Adjust route as needed
            } else {
            ...
            <div className="flex flex-col lg:flex-row w-full max-w-5xl bg-white rounded-2xl shadow-lg overflow-hidden">
                ...
                <div className="lg:w-1/2 w-full p-6 sm:p-10 flex flex-col justify-center">
                    ...
                    <p className="text-center text-sm text-gray-600 mt-4">
                        ...
                        <Link to="/advisor-signup" className="text-blue-600 font-bold">
                            Sign Up
                        </Link>
                    </p>

                    <p className="text-center text-sm mt-2">
                        <Link to="/student-login" className="text-blue-600 font-bold">
                            Login as a student
                        </Link>
                    </p>
